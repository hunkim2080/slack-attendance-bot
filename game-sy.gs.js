...
// 8. 급여 계산
// ==========================================
function calculateDailyPay(totalDays, userType) {
  // userType이 없으면 기본값 "정규직"으로 처리
  userType = userType || "정규직";
  
  // 교육생은 1~60일까지 급여 없음
  if (userType === "교육생" && totalDays <= 60) {
    return 0;
  }
  
  // 교육생이 61일 이상이면 정규 급여 적용
  // 정규직이거나 userType이 없으면 정규 급여 적용
...
function calculateMonthlyPayroll(userName, year, month) {
  try {
    ...
    const userType = userInfo ? (userInfo.user_type || "정규직") : "정규직";
    ...
    uniqueDates.forEach((workDate, index) => {
      ...
      const dailyPay = calculateDailyPay(currentTotalDays, userType);
      totalPay += dailyPay;
      dailyBreakdown.push({
        date: workDate,
